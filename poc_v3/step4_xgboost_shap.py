"""
ST545 POC v3 — Step 4: XGBoost + SHAP (Daily-Level)
=====================================================
Key change from v2: daily-level aggregation (方案 B)
 - Loads sentiment from step0 cache (no duplicate FinBERT)
 - Aggregates articles → daily features per (Date, Ticker):
     sent_mean, sent_std, sent_max, sent_min, news_count,
     + per-publisher sentiment means (pub_XXX_sent)
 - Combines with market technical indicators
 - ~738 daily rows (246 days × 3 tickers)
 - LogReg vs XGBoost + SHAP
"""
...
# Fill NaN publisher-sentiment columns with 0 (no articles from that publisher that day)
pub_sent_cols = [c for c in df.columns if c.startswith('pub_') and c.endswith('_sent')]
df[pub_sent_cols] = df[pub_sent_cols].fillna(0)

# Drop remaining NaN rows (from rolling market features)
...
plt.title('SHAP Feature Importance (XGBoost, Daily-Level, All Tickers)')
plt.tight_layout()
...
plt.title('Mean |SHAP| Feature Importance (Daily-Level)')
plt.tight_layout()
plt.savefig('poc/result/shap_bar.png', dpi=150, bbox_inches='tight')
plt.close()

# Mean |SHAP| values for reporting
...
# ── 5. Sentiment vs Market feature importance comparison ──
...
with open('poc/result/xgboost_shap_results.txt', 'w') as f:
    f.write("ST545 POC v3 Step 4 Results: LogReg vs XGBoost (Daily-Level)\n")
    f.write("=============================================================\n")
    ...
    f.write(f"Text Input: FinBERT sentiment from cache → daily aggregation\n")
...
print("\n[+] POC v3 Step 4 complete. Results in poc/result/")
