"""
ST545 POC v12 (Gated Hybrid) — Step 5: v12 Synergy Analysis
========================================================================
1. Global PCA: Reduce 768-dim FinBERT to 16-dim.
2. Media Weighting: Map publisher weights from Step 3.
3. DQS Gating: (Count/10) * (1 - Std) to assess daily signal quality.
...
print("--- v12: Loading Global Cache & Performing Global 16-dim PCA ---")
...
for ticker in TICKERS:
    ...
    SENT_BASE = ['sent_weighted_mean', 'sent_std', 'DQS']
    X_market = df_t[MARKET_BASE].values
    X_sent = df_t[SENT_BASE + pca_cols].values
...
res_df.to_csv(f'{RESULT_DIR}/hybrid_results_v12.csv', index=False)
with open(f'{RESULT_DIR}/hybrid_report_v12.txt', 'w') as f:
    f.write("ST545 POC v12 Synergy Report - Gating + Media Weight + PCA 16\n" + "="*80 + "\n")
...
print(f"\n[+] v12 complete. Results saved in {RESULT_DIR}")
